...
class HeroesService with ChangeNotifier {
  ...
  /// constructor
  HeroesService({@required this.apiService});

  /// gets or sets the property of all athletes
  List<Benchmarks> _heroes = [];
  List<Benchmarks> get athletes => _heroes;
  ...
  /// gets initial list of heroes limited to 22
  Future<void> getHeroesAsync() async {
    try {
      this.isBusy = true;
      ...
      final response = await apiService.getBenchmarkByCategory("heroes");
  ...
  /// paging next 22 heroes
  Future<void> getNextHeroesAsync() async {
    try {
      this.isBusy = true;

      if (hasNextPage) {
        final response =
            await apiService.getNextBenchmarkByCategory("heroes", _nextPageUrl);
        if (response.statusCode != 200) {
          this.errorMessage = response.error;
          this.hasError = true;
        } else {
